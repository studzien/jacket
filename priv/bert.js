...
function BertClass() {
    ...
    this.PORT = String.fromCharCode(102);
    this.PID = String.fromCharCode(103);
}
...
function BertPid(Node, Id, Serial, Creation) {
    this.type = "Pid";
    this.node = Node;
    this.id = Id;
    this.serial = Serial;
    ...
    this.toString = function() {
        return "<"+this.node.toString()+"."+this.id+"."+this.serial+">";
    };
};

function BertPort(Node, Id, Creation) {
    this.type = "Port";
    this.node = Node;
    this.id = Id;
    this.creation = Creation;
    this.toString = function() {
        return "#Port<"+this.node.toString()+"."+this.id+">";
    };
};
...
BertClass.prototype.pid = function(Node, Id, Serial, Creation) {
    return new BertPid(Node, Id, Serial, Creation);
};


BertClass.prototype.port = function(Node, Id, Creation) {
    return new BertPort(Node, Id, Creation);
};
...
BertClass.prototype.encode_object = function (Obj) {
    ...
    if (Obj.type == "Pid") {
        return this.encode_pid(Obj);
    }
    if (Obj.type == "Port") {
        return this.encode_port(Obj);
    }
...
BertClass.prototype.encode_port = function(Obj) {
    return this.PORT + this.encode_atom(Obj.node) +
           this.int_to_bytes(Obj.id, 4) +
           this.int_to_bytes(Obj.creation, 1);
};

BertClass.prototype.encode_pid = function(Obj) {
    return this.PID + this.encode_atom(Obj.node) +
           this.int_to_bytes(Obj.id, 4) +
           this.int_to_bytes(Obj.serial, 4) +
           this.int_to_bytes(Obj.creation, 1);
};
...
BertClass.prototype.decode_inner = function (S) {
    ...
    case this.PORT:
        return this.decode_port(S);
    case this.PID:
        return this.decode_pid(S);
    default:
...
BertClass.prototype.decode_list = function (S) {
    ...
    for (i = 0; i < Size; i++) {
        ...
        Arr.push(El.value);
        if(El.value.type != "Tuple" || El.value.length != 2) {
            isProplist = false;
        }
        S = El.rest;
    }
...
BertClass.prototype.decode_port = function (S) {
    var Node = this.decode_inner(S);
    S = Node.rest;
    Node = Node.value;
    var Id = this.bytes_to_int(S, 4);
    S = S.substring(4);
    var Creation = this.bytes_to_int(S, 1);
    return {
        value: this.port(Node, Id, Creation),
        rest: S.substring(1)
    };
};

BertClass.prototype.decode_pid = function (S) {
    var Node = this.decode_inner(S);
    S = Node.rest;
    Node = Node.value;
    var Id = this.bytes_to_int(S, 4);
    S = S.substring(4);
    var Serial = this.bytes_to_int(S, 4);
    S = S.substring(4);
    var Creation = this.bytes_to_int(S, 1);
    return {
        value: this.pid(Node, Id, Serial, Creation),
        rest: S.substring(1)
    };
};
